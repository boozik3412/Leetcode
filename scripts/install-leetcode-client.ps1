...
function New-LeetcodeClientShortcut([string]$Path, [string]$Target, [string]$WorkingDirectory) {
...
$exeSource = Join-Path $sourceRoot "leetcode-client.exe"
if (-not (Test-Path -LiteralPath $exeSource)) {
    throw "leetcode-client.exe was not found next to this installer script: $exeSource"
}
...
$exeTarget = Join-Path $installRoot "leetcode-client.exe"
if (-not $NoShortcuts) {
    $startMenuDir = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\Leetcode"
    New-Item -ItemType Directory -Force -Path $startMenuDir | Out-Null
    New-LeetcodeClientShortcut -Path (Join-Path $startMenuDir "Leetcode Client.lnk") -Target $exeTarget -WorkingDirectory $installRoot

    if (-not $NoDesktopShortcut) {
        $desktop = [Environment]::GetFolderPath("Desktop")
        New-LeetcodeClientShortcut -Path (Join-Path $desktop "Leetcode Client.lnk") -Target $exeTarget -WorkingDirectory $installRoot
    }
}
...
Write-Host "Leetcode Client installed:"
