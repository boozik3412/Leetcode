...
The `Карта` workspace turns that graph into an interactive project map. It supports pan/zoom, search, type filters, node selection, pin/hide actions, manual `related_to` links, and double-click opening for file/asset nodes. When a node is selected, the right `Карта` panel shows its description, path, metadata, and incoming/outgoing links. The next agent request automatically receives a compact selected-node context block, so prompts like “улучши это” can refer to the currently selected node without repeating the path.
