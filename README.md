...
The relay also serves an iPhone-friendly PWA at its root URL, for example `http://relay-host:17990/`. In the host app create a pairing code and click `Ссылка iPhone`; the copied link opens the Relay PWA with Agent ID and the temporary code prefilled. After pairing, the PWA stores its device token locally and can show agent status, submit tasks, approve or deny pending plans/actions, browse recent runs/logs, and run safe remote commands. The default mobile roles are view, chat, and approve; file access stays off unless you explicitly enable it for that device in the host app.
