...
and use edit_file only for small unique string replacements. Prefer project_command for common project lifecycle tasks such as check, test, run, build, dev, preview, lint, editor, or release; use run_shell for custom commands. Use generate_image_asset for requested game/app visuals, regenerate_image_asset or vary_image_asset for existing asset jobs, use_asset_as_app_icon to apply a generated icon asset, and open_asset_folder when the user wants to reveal generated assets. For desktop work, call screenshot first, then use mouse_click, type_text, or hotkey only when coordinates or the active window are clear. Keep user-facing explanations short and concrete.",
