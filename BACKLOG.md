...
- Done: Свернуть технический отчёт и диагностику окружения в компактный раздел конструктора, начинать рабочий поток с блока `Требует внимания` и показывать одну строку актуальности Project Map.
...
- Done: Заменить непрозрачный статус `устарела` на контроль реальных изменений: исключить служебные файлы Leetcode из fingerprint, показывать `актуален` без изменений, `есть изменения` только при fingerprint diff и предлагать синхронизацию карты.
- Done: Добавить проектную историю недавно изменённых целей: после успешной изменяющей задачи запоминать точный `node_id/object_path`, поднимать совместимые объекты отдельным блоком `Недавние` и ранжировать по текущей операции, направлению и сфере без дубликатов.
...
## Этап 47 - Semantic Entity Layer

Цель: превратить обезличенные файлы и Unreal-ассеты в объяснимые проектные сущности, чтобы агент предлагал главный Character, HUD, NPC и другие релевантные объекты вместо совпадений только по имени.

- Done: Добавить единую таксономию `domain / system / entity / role / capability / importance / state / scope`, связанную с каталогом 8 x 10 конструктора игровых задач.
- Done: Построить детерминированный анализ по типу узла, object path, Asset Registry metadata, имени и реальным связям Project Map с evidence и confidence.
- Done: Автоматически применять только высокоуверенные выводы, а неоднозначные и AI-метки проводить через proposal и явное подтверждение пользователя.
- Done: Сохранять подтверждённые, заблокированные и отклонённые решения после refresh Project Map; удалённые пользователем метки не предлагать повторно.
- Done: Хранить смысловой индекс и JSONL audit вне Unreal-проекта; экспортировать `.leetcode/semantic-labels.json` только отдельным действием.
- Done: Добавить редактор меток выбранного узла Project Map с поиском, доказательствами, confidence, подтверждением, отклонением, добавлением и удалением.
- Done: Добавить сводку и очередь предложений в конструктор игровой задачи, а автоматический анализ запускать после успешного глубокого Project Map scan.
- Done: Ранжировать совместимые цели в блоки `Рекомендуемые`, `Недавние`, `Связанные` и `Все остальные`, не ослабляя жёсткие TargetContract и TaskManifest.
- Done: Добавить agent tools, строгие provider schemas, governance, audit и явные инструкции не применять AI-предложения без согласия пользователя.
- Done: Добавить регрессионные тесты HUD/Player, несвязанного ассета, relation boost и сохранения пользовательских решений; документировать хранение и безопасность.
